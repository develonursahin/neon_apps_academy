...
//Unwrapping Operations Beginning
void unwrappingOperations() {
  print("\nUnwrapping Operations");
  print("\n[1] Print messages based on member's motivation level.");
  print("\n[2] Find out the degree of motivation level of a member.");
  print("\n[3] Find out a member's motivation level.");
  print(
      "\n[4] Compare a member's target motivation level with their current motivation level.");
  ...
  switch (choice) {
    case "1":
      String customCase = "1";
      switchCaseOptions(customCase);
    case "2":
      determineMotivationLevel();
    case "3":
      String customCase = "3";
      switchCaseOptions(customCase);
    case "4":
...
switchCaseOptions(String customCase) {
  stdout.write("Enter the full name of the member: ");
  String? fullName = stdin.readLineSync();

  if (fullName != null) {
    findMemberMotivation(fullName, customCase);
  } else {
...
void printMotivationMessage(fullName) {
  if (fullName.motivationLevel == null) {
    print("This member has no motivation level set");
  } else if (fullName.motivationLevel! > 5) {
    print("This member is highly motivated");
  }
}

void determineMotivationLevel() {
  print("Enter the motivation level of the member: ");
  int? motivationLevel = int.tryParse(stdin.readLineSync()!);

  if (motivationLevel == null) {
    print("Invalid input. Please enter a valid integer.");
  } else if (motivationLevel > 5) {
    print("The member is highly motivated.");
  } else if (motivationLevel > 0) {
    print("The member is moderately motivated.");
  } else {
    print("The member is not motivated at all.");
  }
}

void findMemberMotivation(String fullName, String customCase) {
  int? motivationLevel;
  for (var member in members) {
    if (member.fullName == fullName) {
      motivationLevel = member.motivationLevel;
    }
  }
  if (customCase == "1") {
    if (motivationLevel == null) {
      ...
      print("This member is low motivated.");
    }
  }
  if (customCase == "3") {
    print("Member $fullName Motivation Level: ${motivationLevel ?? 0}");
  }
  if (customCase == "4") {
    bool status;
    int? targerMotivationLevel;
    stdout.write("Enter the target motivation level: ");
    var targetMotLev = stdin.readLineSync();
    if (targetMotLev != null && targetMotLev.isNotEmpty) {
      targerMotivationLevel = int.parse(targetMotLev);
      if (motivationLevel == null) {
        motivationLevel = 0;
        if (motivationLevel >= targerMotivationLevel) {
          status = true;
          targetMotCurrentMot(status);
        } else {
          status = false;
          targetMotCurrentMot(status);
        }
...
bool targetMotCurrentMot(status) {
  print(status);
  return status;
}
...
void updateMemberLevel(String fullName) {
  ...
  // Update memberLevel based on member's team
  switch (member.team) {
    case Team.FlutterDevelopment:
    case Team.iOSDevelopment:
    case Team.AndroidDevelopment:
...
//General Menu Operations for Unwrapping
void menuOptions2() {
  stdout.write(
      'Enter [b] to return to the previous menu, [m] to return to the main menu, or [q] to exit: ');
  var choice = stdin.readLineSync()!;
...
newMember() {
  ...
  stdout.write('Motivation Level: ');
  String? motivationInput = stdin.readLineSync();
  if (motivationInput != null && motivationInput.isNotEmpty) {
    motivationLevel = int.parse(motivationInput);
  }
  stdout.write('Email: ');
...
memberList() {
  for (int i = 0; i < members.length; i++) {
    ...
    print("Age: ${members[i].motivationLevel}");
